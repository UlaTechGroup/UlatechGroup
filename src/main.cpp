...
#if defined(NDEBUG)
# error "UlaTech cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1b12b0f061a5e8313157563b86bef7d73b45dc6668a084b9b4c1615c9c9d9a34");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UlaTech: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UlaTech Signed Message:\n";
...
{
    // UlaTech: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 25 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 54000000 * COIN;
            }

    // Subsidy is cut in half every 720000 blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // UlaTech: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // UlaTech: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1503306012, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Royals come back 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0423c5c239c335c2dc570e84b733db0a153f44ce6ec8d7f483c0bef0b61dbc737d5a150df22c679f523925f0d49673c25e7b4de4eddd666f8ea75593e18514310e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503306012;
        ...
        assert(block.hashMerkleRoot == uint256("0xc667163aaf8fa0e3aa346783081f25fa5c3a3672c254d7bcca86adfe2261e80e"));
...
unsigned char pchMessageStart[4] = { 0x8b, 0xa3, 0x36, 0x9a }; // UlaTech: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UlaTechMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UlaTechMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UlaTechMiner(CWallet *pwallet)
{
    printf("UlaTechMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ulatech-miner");
    ...
    try { loop {
        ...
        printf("Running UlaTechMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UlaTechMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UlaTechMiner, pwallet));
}
